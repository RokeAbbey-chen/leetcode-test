...
//        if (len <= 0) return null;
        if (len <= 3 || isAllLine(trees)) return trees;
        double[][] trees2 = new double[len][];
        double[][] thetas = new double[len][2];
        ...
        for (int i = 0; i < len; i ++) {
            trees2[i] = new double[]{trees[i][0] - meanX, trees[i][1] - meanY, i};
        }

        Comparator<double[]> cmp0 = (o0, o1)-> {
            double len0 = sqLen(o0);
            double len1 = sqLen(o1);
            if (len0 < len1) return 1;
            if (len0 > len1) return -1;
            return 0;
        };
        Arrays.sort(trees2, cmp0);

        for (int i = 0; i < len; i ++) {
            if (0 == trees2[i][0] && 0 == trees2[i][1])
                thetas[i][0] = 0;
            else {
                thetas[i][0] = Math.acos(trees2[i][0] / Math.sqrt(sqLen(trees2[i])));
                if (trees2[i][1] < 0)
                    thetas[i][0] = 2 * Math.PI - thetas[i][0];
                thetas[i][0] = thetas[i][0] * 180 / Math.PI;
            }
            thetas[i][1] = i;
        }

        Comparator<double[]> cmp1 = (o0, o1) -> {
            if (fe(o0[0], o1[0], 1e-6)) return 0;
            if (o0[0] > o1[0]) return 1;
            return -1;
        };
        TreeSet<double[]> vertexes = new TreeSet<>(cmp1);

        for (int i = 0; i < len; i ++) {
            if (vertexes.size() <= 0) {
                vertexes.add(thetas[i]);
                continue;
            }
            double[] old = vertexes.floor(thetas[i]);
            if (null != old && fe(old[0], thetas[i][0], 1e-6)){
                if (sqLen(trees2[(int)old[1]]) <= sqLen(trees2[(int)thetas[i][1]])){
                    vertexes.add(thetas[i]);
                } else {
//                    System.out.println("skip 0");
                ...
                double[] floor = old, ceil = vertexes.ceiling(thetas[i]);
                if (null == ceil || null == floor) {
                    floor = vertexes.first();
                    ceil = vertexes.last();
                }

                if (thetas[i][0] >= floor[0] && thetas[i][0] <= ceil[0] && ceil[0] - floor[0] >= 180 - 1e-8
                        || thetas[i][0] < Math.min(floor[0], ceil[0]) && ceil[0] - floor[0] <= 180 + 1e-8
                        || thetas[i][0] > Math.max(floor[0], ceil[0]) && ceil[0] - floor[0] <= 180 + 1e-8) {
                    vertexes.add(thetas[i]);
                } else {
                    boolean n = crossN(trees2, (int) thetas[i][1], (int) floor[1], (int) ceil[1]);
                    if (n) {
                        vertexes.add(thetas[i]);
                    } else {
//                        System.out.println("skip 1");
        ...
        for (double[] v: vertexes) {
            result[i++] = trees[(int)trees2[(int)v[1]][2]];
        }
        return result;

    }

    private static double sqLen(double[] o) {
        return o[0] * o[0] + o[1] * o[1];
    }
    private boolean fe(double x, double y, double precious) {
        return Math.abs(x - y) <= precious;
    }

    private boolean crossN(double[][] tree, int index, int start, int end) {
        double x0 = tree[index][0], y0 = tree[index][1];
        double x1 = tree[start][0], y1 = tree[start][1];
        double x2 = tree[end][0], y2 = tree[end][1];

        double num0 = -x2 * y0 + x0 * y2 + y0 * x1 - x0 * y1;
        if (fe(num0, 0, 1e-6)) return false;
//        if (0 == num0) return 0;
        double num1 = x1 * y2 - x2 * y1;
        if (fe(num1, 0, 1e-6)) return true;
        return num1 / num0 <= 1 + 1e-6;
    }

    private boolean isAllLine(int[][] tree) {
        int[] base = tree[0];
        int x = tree[1][0] - base[0], y = tree[1][1] - base[1];
        for (int i = 2; i < tree.length; i ++) {
            int xi = tree[i][0] - base[0], yi = tree[i][1] - base[1];
            if (yi * x != xi * y) return false;
        }
        return true;
    }

    public static void main(String[] args) {
        Test587 t = new Test587();
        int[][] tree = new int[][] {
                {1, 0}, {2, 0},
                {3, 1}, // x
                {4, 1}, {5, 2},
    ...
    private static void paint(int[][] ps) {
        ...
        for (int y = maxY; y >= minY; y --) {
            for (int x = minX; x <= maxX; x ++) {
                if (i < ps2.length && ps2[i][0] == x && ps2[i][1] == y){
