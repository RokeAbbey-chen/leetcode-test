...
public class Test587_3 {
    public int[][] outerTrees(int[][] trees) {
        ...
        boolean[] bVertex = new boolean[n];
        int li = 0;
        for (int i = 1; i < n; i ++) {
            if (trees[i][0] < trees[li][0]) {
                li = i;
            }
        }
        bVertex[li] = true;
        ...
        while (true) {
            ...
            bases.add(k);
            for (int i = 0; i < n; i++) {
                if (i == li || i == base) continue;
//                System.out.println("li = " + li + ", i = " + i + ", base = " + base);
//                System.out.println("(" + trees[li][0] + ", " + trees[li][1] + "), (" + trees[i][0] + ", " + trees[i][1] + "), (" + trees[base][0] + ", " + trees[base][1] + ")");
                int cross = crossProduct(trees[li], trees[i], trees[base]);
                if (cross > 0) {
                    bases.clear();
                    base = i;
                    bases.add(base);
...
//                        bases.add(base);
    ...
    public static int crossProduct(int[] xy0, int[] xy1, int[] xy2) {
        // xy2 = base
        return (xy2[0] - xy0[0]) * (xy1[1] - xy0[1])
                - (xy2[1] - xy0[1]) * (xy1[0] - xy0[0]);
    }

    public static double sqDis(int[] xy0, int[] xy1) {
        int dx = xy0[0] - xy1[0];
        int dy = xy0[1] - xy1[1];
        return dx * dx + dy * dy;
    }
